...
QUnit.test("retrieving the dialog", function(assert) {

    $('#fixture').append('<div id="myModal" />');
    game.init({});
    assert.ok(game.getDialog().hasClass('in'));
});

QUnit.test("test Timer start", function(assert) {
    game.init({});
    assert.equal(game.components.timer, null);
    game.startTimer();
    assert.notEqual(game.components.timer, null);
});

QUnit.test("test Timer add", function(assert) {
    game.init({});
    assert.equal(game.data.elapsedTime, 0);
    game.addTimer();
    assert.equal(game.data.elapsedTime, 1);
});
