var game = {
	...
	if (!flip.isFlipped && game.data.openCards.length < 2) {
		...
		tile.find('.card-front-img').css('display', 'block').attr('src', data.card.image);
		// .css('background-image',"url('" + data.card.image +
		// "')");
	    }
	    else {
		tile.find('.card-front-img').css('display', 'none')
		tile.find('.card-front-text').css('display', 'table-cell').html(data.card.word);
		...
		var audiofile = 'data/audio/' + game.data.language + '/' + data.card.word + '.mp3';
